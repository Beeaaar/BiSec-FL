...
class RLWE:
    ...
    def encrypt(self, m, pub):
        ...
        return (m + b.poly.coeffs.tolist()[0] * self.v + self.t * e[2], a.poly.coeffs.tolist()[0] * self.v + self.t * e[1])
    ...
    def decrypt(self, c, s,e_star):
        ...
        '''
        #e_star = discrete_gaussian(self.n,self.p,self.std)
        pardec = c * s + e_star
...
def discrete_uniform(n, q, min=0., max=None):
    if max is None:
        max = q
    coeffs = np.random.randint(min, max, size=n,dtype=np.int64)
    return Rq(coeffs, q)