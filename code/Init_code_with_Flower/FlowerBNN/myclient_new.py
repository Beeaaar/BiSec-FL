...
net = Binary_MLP(classes=10,WEIGHT_DECIMALS=7).to(DEVICE)
...
class MNISTClient(fl.client.NumPyClient):
    ...
    def set_parameters(self,bias,weights):
        #set_weight(self.model,parameters)
        set_weight_and_bias(self.model,weights,bias)
    ...
    def get_binary_weight(self):
        ...
        for name, val in self.model.named_parameters():
            if 'weight' in name:
                weights.append(binarized(val).cpu().detach().numpy())
    ...
    def fit(self,parameters,config):
        ...
        train_model(self.model,trainloader,testloader,testloader,DEVICE,epochs=1,lr = 1e-3)
        Loss,Acc = test_model(self.model,testloader,DEVICE)
        ...
        return self.get_binary_weight(), len(trainloader.dataset), {}
    
    #好像没用到这个evaluate函数
    def evaluate(self, parameters, config):    
        self.set_parameters(parameters)
        loss, accuracy = test_model(self.model, testloader,DEVICE) 
    ...
    def receive_updated_weights(self, server_flat_para) -> bool:
        ...
        self.set_parameters(server_bias,server_weights)
    ...
    def test_server_model(self):
        ...
        with open('./results/Secure/MNISTBMLP{}.txt'.format(args.dataidx), 'a') as file:
                file.write(str("{:.4f}".format(Loss))+' '+str("{:.2f}".format(Acc)) + '\n')

        if Acc > self.Acc:
            self.Acc = Acc
            pth = r'./results/Secure'
            torch.save(self.model.state_dict(), pth+'/BMLP'+'.pth')
