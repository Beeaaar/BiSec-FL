...
parser.add_argument('--datapth',metavar='DATA_PATH',
                    default='D:/Graduation project/Code/FlowerBNN/data/MNIST/client1',
                    help = 'data_path')
...
net = Binary_CF_Mini(WEIGHT_DECIMALS=6).to(DEVICE)
def Init_RLWE(Scale,num_clients):
    ...
    #t = 200_000_001
    #std = 3
    t = utils.next_prime(num_clients*(10**7)*3)
...
class MNISTClient(fl.client.NumPyClient):
    ...
    def set_parameters(self):
        set_weight_and_bias(self.model,self.weight,self.bias)
    ...
    def encrypt_parameters(self,request):
        ...
        #这里如果分批处理了，需要改
        flattened_bias, self.bias_shape = utils.get_flat_bias(self.bias)
        flattened_bias, self.model_length = utils.pad_to_power_of_2(flattened_bias, self.rlwe.n, self.WEIGHT_DECIMALS)
        ...
        poly_bias = Rq(np.array(flattened_bias), self.rlwe.t)
        #暂时用不到这个gradients，如果需要还要重新处理
        '''if request == "gradient":
            gradient = list(np.array(flattened_bias) - np.array(self.flat_params))
            #print(f"Client gradient: {gradient[925:935]}")
            poly_bias = Rq(np.array(gradient), self.rlwe.t)'''
        c0, c1 = self.rlwe.encrypt(poly_bias, self.allpub)
        c0 = list(c0.poly.coeffs)
        c1 = list(c1.poly.coeffs)
        print("In myclient:147,plain_text(bias):",self.model_length,len(flattened_bias),poly_bias.poly_to_list()[:8])
        print(f"c0: {c0[:8]}")
        print(f"c1: {c1[:8]}")
        return c0, c1
    ...
    def compute_decryption_share(self,csum1):
        ...
        print("In myclient:158,Compute D1:",d1[:8])
        return d1
    
    #看看server怎么传回来的
    def receive_updated_weights(self, agg_para) -> bool:
        # Convert list of python integers into list of np.64
        #agg_para中前6346是bias，填充到2^13，后面的是weight，想办法恢复到网络里
        flat_bias = list(np.array(agg_para[:(1<<13)],dtype=np.float32))
        #print(len(flat_bias),flat_bias[:10])
        print(f"In myclient:169,Fedavg plaintext(para or gra): {flat_bias[:8]}")
        self.bias = utils.unflatten_weights(flat_bias, self.bias_shape)
        
        self.flat_params = agg_para[(1<<13):]
        self.weight = utils.unflatten_weights(self.flat_params,self.weight_shape)
        ...
        '''server_flat_weights = list(np.array(server_flat_weights, dtype=np.float32))
        ...
        print(f"In myclient:153,Fedavg plaintext(para or gra): {server_flat_weights[:8]}")

        self.set_parameters(server_weights)'''
