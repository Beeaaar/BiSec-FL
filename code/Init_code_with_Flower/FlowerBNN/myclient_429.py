...
class MNISTClient(fl.client.NumPyClient):
    ...
    def set_parameters(self,bias):
        ...
        set_bias(self.model,bias)
        '''params_dict = zip(net.state_dict().keys(), parameters)
        state_dict = OrderedDict({k: torch.tensor(v) for k, v in params_dict})
        self.model.load_state_dict(state_dict, strict=True)'''
    #这个设置很神奇，应该是在fit之前就专门传过参数把网络设置好了，所以这里没给参数，直接网络去训练就行了
    def set_weight_meaning(self,para):
        server_weights = utils.unflatten_weights(para,self.weight_shape)
        set_weight_and_meaning(self.model,server_weights)
    ...
    def fit(self,parameters,config):
        ...
        if parameters != []:
            self.set_weight_meaning(parameters)
    ...
    def receive_updated_weights(self, server_flat_bias) -> bool:
        ...
        server_flat_bias = list(np.array(server_flat_bias, dtype=np.float32))
        ...
        self.set_parameters(server_bias)
