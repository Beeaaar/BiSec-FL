...
class Server:
    ...
    def Get_data_num_ratio(self,data_num,Max=10)-> List:
    ...
    def fit(self, num_rounds: int, timeout: Optional[float]) -> History:
        ...
        print("It Looks like we don't have Initial parameters?",res)
        if res is not None:
            log(
                INFO,
        ...
        print("INFO HE Public Key Aggregation/Distribution Time:", execution_time)
        ...
        for current_round in range(1, num_rounds + 1):
            ...
            res_fit,agg_weight = self.fit_round(server_round=current_round, timeout=timeout)
            for _,fitr in res_fit:
                print("\nI got DATA_NUM:",fitr.num_examples)
                print("AGG_WEIGHY",len(agg_weight),type(agg_weight),agg_weight[:4])
                #print("I GOT WEIGHT",parameters_to_ndarrays(fitr.parameters))
            ...
            # After round, clients encrypt plaintext p -> (c0, c1) and here we sum up all c0 and c1 polynomials
            print("In server,next step is request_encrypted_paras:",request)
            clients = list(self._client_manager.clients.values())
            #print("data_num_ratio:",data_num_ratio,sum(data_num_ratio))
            ...
            print("In server:255,c0sum:",c0sum.poly_to_list()[:10])
            print("In server:256,dsum:",dsum.poly_to_list()[:10])
            plaintext = plaintext.testing(self.strategy.rlwe.t)
            plaintext = plaintext.poly_to_list()

            L = len(clients)#*data_num_ratio
            avg_weights = [round(weight/L) for weight in plaintext]
            for c in clients:
                confirmed = c.request_modelupdate_confirmation(avg_weights+agg_weight, timeout=timeout)
    ...
    def fit_round(
        ...
        return results,aggregated_result
        #自己改的
